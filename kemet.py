...
_gapfill_modes = ["existing","denovo","onlyfasta"]
...
def eggnogXktest(eggnog_file, converted_output, ktests_directory):
    """
    ...
    converted_output = converted_output + ".ktest" # guido
    KOs = {}
...
def KAASXktest(file_kaas, converted_output, ktests_directory):
    """
...
def kofamXktest(kofamkoala_file, converted_output, ktests_directory):
    """
...
def create_tuple_modules_1BM(fasta_id, oneBM_modules_dir, report_tsv_directory):
    """
    ...
    # guido start
    try:
        with open("reportKMC_"+fasta_id+".tsv") as f:
            for line in f.readlines():
        ...
        raise("tsv report for", fasta_id, "not found")
    # guido end

    try:
        os.chdir(oneBM_modules_dir)
    except:
        os.mkdir(oneBM_modules_dir)
        os.chdir(oneBM_modules_dir)
    m = open(fasta_id+"_module_file.instructions", "w")
...
def write_KOs_from_modules(fasta_id, tuple_modules, report_txt_directory, klists_directory):
    ...
    # guido start
    try:
        with open("reportKMC_"+fasta_id+".txt") as f:
            klist = []
            v = f.readlines()
            f.seek(0)
            i = 0
            ...
            g = open(fasta_id+".klist", "w")
            for KO in klist:
                g.write(KO+"\n")
            g.close()
            os.chdir(report_txt_directory)
    except:
        raise("txt report for", fasta_id, "not found")
    # guido end        
...
def taxonomy_filter(taxonomy, dir_base, taxa_file, taxa_dir, update = False):
    ...
    """
    os.chdir(dir_base)
    os.system(f"curl --silent https://rest.kegg.jp/get/br:br08601 > br08601.keg")
    taxa_allow=[]
    with open("br08601.keg","r") as f:
        v = f.readlines()
        f.seek(0)
        i = 0
        i_start=0
...
def getntseq(gene):
    ...
    os.system(cmd_get_ntseq+" > "+gene_taxa+".fna")
    return 1
    ...
    for genome, val in MAG_Khit_dict.items():
        for hits in MAG_Khit_dict[genome]:
            ...
            for line in v_lines:
                line = line.strip()
                i += 1
                if (">" in line) and (fragment in line):
                    i_frag_start = i
                    j = i
                    break
            for line in v_lines[i_frag_start+1:]:
                j += 1
                if ">" in line: # new contigs or file-end
                    i_frag_stop = j
                    break
                elif j >= (len(v_lines)-1):
                    i_frag_stop = j+1
                    break
...
def build_de_novo_GSMM(fasta_genome, de_novo_model_directory, current_run, proteome, just_expand_fasta=False, log=False): # guido
    """
    ...
    Args:
        ...
        log                  (bool, optional): keep execution times in a log file (if specified in command-line args). Defaults to False.
    """
    fasta_id = os.path.basename(fasta_genome).replace(".fasta","").replace(".fna","").replace(".fa","") # guido
    HMM_HITS = {}
    ...
    if proteome == None:
        ...
        print(_timeinfo()+"COMPLETE Prodigal command for "+fasta_genome)
        if log == True:
            logging.info('COMPLETE Prodigal command for '+fasta_genome)
        ...
        os.chdir(de_novo_model_directory+"proteins")
        os.system("cp " + proteome + " " + os.path.basename(proteome).replace(".faa", "_expanded.faa"))
        f = open(os.path.basename(proteome).replace(".faa", "_expanded.faa"), "a")
        for ko, xseq in HMM_HITS.items():
            f.write(ko+"\n")
            f.write(xseq+"\n")
        f.close()
    ...
    if not just_expand_fasta:
        ...
        if proteome == None:
            ...
            carveme_cmd_mod = carveme_cmd.replace("FASTA",fasta_id+"_expanded").replace("universe",metabolic_universe)
        print(_timeinfo()+"START CarveMe command for "+fasta_id)
        if log == True:
            logging.info('START CarveMe command for '+fasta_id)
        os.system(carveme_cmd_mod)
        print(_timeinfo()+"COMPLETE CarveMe command for "+fasta_id)
        if log == True:
            logging.info('COMPLETE CarveMe command for '+fasta_id)
...
if __name__ == "__main__":
    ...
    denovo: generate a new CarveMe GSMM, performing gene prediction and adding HMM-derived hits from the chosen HMM-mode.
                        ''')
    parser.add_argument('--protein_fasta', default = None,
                        help='''Proteome FASTA file (.faa) for the input genome.''')
    parser.add_argument('-o','--path_output',
                        help='''Absolute path to ouput file(s) FOLDER.''', default = os.getcwd())
    ...
    ###############
    # guido start
    fasta_file_name = os.path.basename(args.FASTA_file)
    if fasta_file_name.endswith(".fasta"):
        fasta_extension = ".fasta"
    elif fasta_file_name.endswith(".fna"):
        fasta_extension = ".fna"
    elif fasta_file_name.endswith(".fa"):
        fasta_extension = ".fa"
    fasta_file_name = fasta_file_name.replace(fasta_extension, "")
    fasta_full_path = os.path.abspath(args.FASTA_file)
    annotation_file_name = os.path.basename(args.annotation_file)
    annotation_full_path = os.path.abspath(args.annotation_file)
    protein_fasta_full_path = args.protein_fasta
    if protein_fasta_full_path is not None:
        protein_fasta_full_path = os.path.abspath(protein_fasta_full_path)
    kemet_dir = os.path.dirname(os.path.realpath(__file__)) + "/"

    Modules_directory = kemet_dir+"KEGG_MODULES/"
    kkfiles_directory = Modules_directory+"kk_files/"
    taxa_dir = kemet_dir+"taxonomies/"
    dir_base_KO = kemet_dir+"Knumber_ntsequences/"
    bigg_api = kemet_dir+"biggapi_download/"
    ...
    klists_directory = dir_output+"/klists/"
    msa_dir = dir_output+"/MSA/"
    hmm_dir = dir_output+"/HMM/"
    hmm_hits_dir = dir_output+"/HMM_HITS/"
    oneBM_modules_dir = dir_output+"/oneBM_modules/"
    gapfill_report_directory = dir_output+"/report_gapfill/"
    model_directory = dir_output+"/models/"
    ...
    if not os.path.exists(report_tsv_directory):
        os.mkdir(report_tsv_directory)
    if not os.path.exists(ktests_directory):
        os.mkdir(ktests_directory)
    if not os.path.exists(klists_directory):
        os.mkdir(klists_directory)
    ...
    if not args.skip_hmm:
        if not os.path.exists(hmm_dir):
            os.mkdir(hmm_dir)
        if not os.path.exists(hmm_hits_dir):
            os.mkdir(hmm_hits_dir)
        if not os.path.exists(oneBM_modules_dir):
            os.mkdir(oneBM_modules_dir)
    if not args.skip_gsmm:
        if not os.path.exists(gapfill_report_directory):
            os.mkdir(gapfill_report_directory)
        if not os.path.exists(model_directory):
            os.mkdir(model_directory)
        if not os.path.exists(gapfilled_model_directory):
            os.mkdir(gapfilled_model_directory)
        if not os.path.exists(de_novo_model_directory):
            os.mkdir(de_novo_model_directory)
    # guido end

    #####################
    # INSTRUCTION_FILES #
    #####################
    instruction_file = dir_output + "/genomes.instruction"
    fixed_module_file = dir_output + "/module_file.instruction"
    fixed_ko_file = dir_output + "/ko_file.instruction"
...
#### KMC - PRODUCE KTEST FILE
    if args.log:
        logging.info('+++\tSTART '+fasta_file_name)
        logging.info('+++START KEGG Modules completeness')
    # guido start
    if args.annotation_format == "kaas":
        if args.verbose:
            print("converting kaas-like file "+annotation_file_name)
        ktest, KOs = KAASXktest(annotation_full_path, fasta_file_name, ktests_directory)
    elif args.annotation_format == "eggnog":
        if args.verbose:
            print("converting eggnog file "+annotation_file_name)
        ktest, KOs = eggnogXktest(annotation_full_path, fasta_file_name, ktests_directory)
    elif args.annotation_format == "kofamkoala":
        if args.verbose:
            print("converting kofamkoala file "+annotation_file_name)
        ktest, KOs = kofamXktest(annotation_full_path, fasta_file_name, ktests_directory)
    else:
        raise('''
    ...
    if ktest in sorted(os.listdir()):
        ko_list = create_KO_list(ktest, ktests_directory) # redundant
        os.chdir(kkfiles_directory)
        for file in sorted(os.listdir()):
            if file.endswith(".kk"):
                testcompleteness(ko_list, file, kkfiles_directory, report_txt_directory, "reportKMC_"+fasta_file_name+".txt") # guido
                testcompleteness_tsv(ko_list, file, kkfiles_directory, report_tsv_directory, "reportKMC_"+fasta_file_name+".tsv", as_kegg = args.as_kegg) # guido
        if args.log:
        ...
        for line in f.readlines()[1:]:
            if line.startswith(fasta_file_name+fasta_extension): # guido
                ...
                if len(line) == 3:
                    ...
                    metabolic_universe = 'bacteria' # guido

                klist_file = fasta_file_name+".klist"
                taxonomy = line[1]
                ...
                if args.hmm_mode == "onebm":
                    tuple_modules = create_tuple_modules_1BM(fasta_file_name, oneBM_modules_dir, report_tsv_directory) # guido
                    write_KOs_from_modules(fasta_file_name, tuple_modules, report_txt_directory, klists_directory)
                if args.hmm_mode == "kos":
                    write_KOs_from_fixed_list(fasta_file_name, fixed_ko_file, ktests_directory, klists_directory)

                dir_KO = dir_base_KO+taxonomy.replace(" ","_")+"/"                           # KO folder for taxonomy,    save time!
                msa_dir_comm = msa_dir+fasta_file_name+"/"                                          # MSA folder for MAG/Genome, more ordered!
                hmm_dir_comm = hmm_dir+fasta_file_name+"/"                                          # HMM folder for MAG/Genome, more ordered!
                CORR_THRESHOLD = float(args.threshold_value)

#### HMM - OPERATE SINGLE FUNCTIONS
                print(_timeinfo()+"+++START HMM operations "+fasta_file_name)
                if args.log:
                    logging.info('+++START HMM operations '+fasta_file_name)
                if args.update_taxonomy_codes:
                    taxa_allow = taxonomy_filter(taxonomy, dir_output, taxa_file, taxa_dir, update = True)
                else:
                    os.chdir(taxa_dir)
                    if not taxa_file in os.listdir():
                        taxa_allow = taxonomy_filter(taxonomy, dir_output, taxa_file, taxa_dir, update = True)
                    else:
                        taxa_allow = taxonomy_filter(taxonomy, dir_output, taxa_file, taxa_dir)
                ...
                if not args.skip_msa_and_hmmbuild:
                    ...
                    filter_and_allign(taxa_dir, taxa_file, fasta_file_name, klist_file, klists_directory, msa_dir, dir_KO)
                    if args.log:
                        logging.info('COMPLETE Filter and allign')
                    MSA_and_HMM(msa_dir_comm, base_com_mafft, base_com_hmmbuild, log=args.log) # guido
                nhmmer_for_genome(fasta_full_path, msa_dir_comm, base_com_nhmmer)
                if args.log:
                    logging.info('COMPLETE nhmmer')
                move_HMM_and_clean(hmm_dir_comm, msa_dir_comm)

#### HMM - FIRST REPORT FILE
                nhmmer_significant_hits_corr(fasta_file_name, hmm_dir_comm, corr_threshold = CORR_THRESHOLD)
                if args.log:
...
#### HMM - TOTAL REPORT FILE
                recap_hits_corr(fasta_file_name, hmm_hits_dir, HMM_hits_dict, HMM_hits_longestTRANSLATED_dict, run_start)
...
#### GSMM - OPERATE SINGLE FUNCTIONS
                        if args.log:
                            logging.info('+++START GSMM operations '+fasta_file_name)

                        fastakohits = fasta_file_name+"_HMM_hits.txt"
                        current_run = "KEMET_run_"+run_start

                        if args.gsmm_mode == "onlyfasta":
                            build_de_novo_GSMM(fasta_full_path, de_novo_model_directory, current_run, protein_fasta_full_path, just_expand_fasta=True, log=args.log) # guido
                        elif args.gsmm_mode == "denovo":
                            build_de_novo_GSMM(fasta_full_path, de_novo_model_directory, current_run, protein_fasta_full_path, just_expand_fasta=False, log=args.log) # guido
                            ...
                            if args.hmm_mode == "modules":
                                MODofinterest = fixed_modules_of_interest(dir_output, fixed_module_file)
                            if args.hmm_mode == "onebm":
                                MODofinterest = onbm_modules_of_interest(fasta_file_name, oneBM_modules_dir)
...
#### GSMM - REACTION ADDITION + RECAP
                            ...
                            log_bigg_nr(bigg_nonredundant, fasta_file_name, gapfill_report_directory)
                            reframed_reaction_addition(fasta_file_name, model_directory, gapfill_report_directory, bigg_api, verbose = args.verbose) # guido
                            recap_addition(fasta_file_name, gapfill_report_directory, old_new_names_R)

                print(_timeinfo()+"END "+fasta_file_name+"\n")
                if args.log:
                    logging.info('END '+fasta_file_name+'\n')
